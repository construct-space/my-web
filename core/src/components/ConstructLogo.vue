...
/**
 * ConstructLogo — the real Construct brand mark.
 *
 * Traced from the same 533×533 SVG the accounts service has shipped in
 * its sidebar and login pages; we carry it here so every surface in my
 * (login, register, consent, oauth callback, shell, 404) renders the
 * identical mark without reaching into another repo.
 *
 * Colour follows the CSS var `--app-accent` by default; override with
 * the `color` prop if the mark should live on a coloured background.
 */
defineProps<{
  size?: number | string
  color?: string
...
<style scoped>
svg { color: var(--app-accent); display: inline-block; vertical-align: middle; }
</style>
